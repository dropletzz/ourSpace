...
setInterval(() => {
    const me = myId ? people[myId] : null;
    if (me) {
        const xDist = Math.abs(me.x - prevX);
        const yDist = Math.abs(me.y - prevY);
        const moved = xDist > 0.0000001 || yDist > 0.0000001;
        console.log('xd', xDist);
        console.log('yd', yDist);
        if (moved) {
            console.log('moved!');
            prevX = me.x;
            prevY = me.y;
            socket.send(JSON.stringify({ act: 'move', x: me.x, y: me.y }));
        }
    }
}, 1000/30);
